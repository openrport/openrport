...
func (s *ClientRepository) Save(client *Client) error {
	s.logger.Debugf("saving client: %s status=%s", client.ID, FormatConnectionState(client.GetDisconnectedAt()))
...
func (s *ClientRepository) Delete(client *Client) error {
	s.logger.Debugf("deleting client: %s status=%s", client.ID, FormatConnectionState(client.GetDisconnectedAt()))
...
func (s *ClientRepository) DeleteObsolete() ([]*Client, error) {
	...
	for _, client := range s.clients {
		if client.Obsolete(s.KeepDisconnectedClients) {
			s.logger.Debugf("deleting obsolete client: %s status=%s", client.ID, FormatConnectionState(client.GetDisconnectedAt()))
