...
func (suite *CleanerTestSuite) SetupTest() {
	var err error
	suite.db, err = sqlite.New(":memory:", me.AssetNames(), me.Asset, sqlite.DataSourceOptions{})
	suite.NoError(err)
	suite.repository = me.NewRepository(suite.db)
...
func (suite *CleanerTestSuite) TestCleanerCleansNothingWhenNothing() {
	repository := me.NewRepository(suite.db)
	c := me.StartCleaner(suite.logger, repository, time.Second, time.Second)
	defer c.Close()
...
func (suite *CleanerTestSuite) startCleaner() me.Closeable {
	repository := me.NewRepository(suite.db)
	c := me.StartCleaner(suite.logger, repository, time.Second, time.Second)
	return c
}
